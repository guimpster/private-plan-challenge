...
const appModule = 'NestJS';
const printContext = (ctx) => (ctx ? `[${ctx}]` : '[NestServer]');
...
export const nestPinoLogger = pino({
  level: process.env.LOG_LEVEL ?? 'debug',
  formatters: {
    // add a derived field that the transport can print
    log(obj) {
      const ctx = obj.context ? `[${obj.context}]` : '[NestServer]';
  ...
  transport: {
    ...
    options: {
      ...
      messageFormat: '[NestJS] {pid}\t - {time}   {contextStr} {msg}',
    },
  },
})